...
- 生成时间：2026-09-06T07:19:29.380Z
- 语料：1,000,000 帧 × 3 DBC（确定性 PRNG seed 0xBEEF，可复现）
- 分块大小（IPC 事件流）：100,000 帧；环境：Node v22.22.2 / win32 x64
...
- ASC 语料文件：C:\Users\jj_10\AppData\Local\Temp\cla-r2ui-EFfIP6\cla-r2ui-1000000.asc
...
| 步骤 | 耗时 | 说明 |
| --- | --- | --- |
| generateASC (1M-frame corpus text) | 2.41 s | 50.4 MB |
| write ASC corpus to disk | 0.09 s |  |
| main-process streaming ASC load | 3.83 s | 解析 1,000,000 帧 |
| saveCompressed (.gz sidecar) | 2.13 s | 50.4 MB → 16.2 MB (32.1%)；heap +-425 MB |
| cache-hit gate (existsSync, no re-compress) | 0.00 s | 命中（0 重复压缩） |
| chunked decode + IPC payload — dbc_full (ext/mux/float/attr) | 3.44 s | 解码 555,556 / 输出 555,556 行；0.29M 帧/s；IPC 负载 52.3 MB；heap +129 MB |
| chunked decode + IPC payload — powertrain | 3.22 s | 解码 444,444 / 输出 444,444 行；0.31M 帧/s；IPC 负载 68.0 MB；heap +102 MB |
| chunked decode + IPC payload — body_chassis | 2.21 s | 解码 444,444 / 输出 444,444 行；0.45M 帧/s；IPC 负载 111.2 MB；heap +115 MB |
| 峰值堆内存 | 935.9 MB | 主进程 heapUsed 峰值采样 |
| **总计** | 17.67 s | 不含人工观测的渲染/绘制 |
